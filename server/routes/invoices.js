    ...
    if (isPostgres) {
      statsQuery = `
        SELECT 
          ...
          COALESCE(SUM(CASE WHEN status = 'emessa' AND "dueDate" < CURRENT_DATE THEN amount ELSE 0 END), 0) as overdueAmount,
          COALESCE(SUM(CASE WHEN status = 'emessa' AND "dueDate" >= CURRENT_DATE THEN amount ELSE 0 END), 0) as pendingAmount,
          ...
          COUNT(CASE WHEN status = 'emessa' AND "dueDate" < CURRENT_DATE THEN 1 END) as overdueCount
        FROM invoices
      `;
    } else {
      statsQuery = `
